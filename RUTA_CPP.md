...
- [ ] **Queda abierta:** `cima()` devuelve `T&` al elemento dentro del array, así que la referencia puede quedar **colgando** si la pila se vacía o se recicla (misma trampa de la Sesión 20 con otro disfraz). ¿Qué garantiza `std::stack::top()` y bajo qué regla el retorno por referencia es seguro? (preguntada al cierre de la Sesión 26, sin respuesta todavía)

**Estado:** En curso (Sesión 26: `Pila<T>` sobre array completa y validada; faltan el reto de paréntesis, `std::stack`, infija→postfija, la versión con lista enlazada y la pregunta de la referencia colgante)
