...
- [x] Lista circular — `ListaCircular.cpp` (Sesión 23): lista simplemente enlazada cuyo último nodo apunta a la cabeza (`ultimo->siguiente = cabeza`) en vez de `nullptr`. Nodo único se apunta a sí mismo; lista vacía = `cabeza == nullptr`. `insertarFinal`/`insertarInicio` (los tres casos: vacía, un nodo, varios), `buscar`, `eliminar` (cabeza/intermedio/cola/último nodo), `imprimir` que repite la cabeza para mostrar el ciclo, destructor RAII y copia deshabilitada. Compilación limpia; valgrind 7 allocs/7 frees, 0 leaks, 0 errores. `insertarFinal`/`insertarInicio` son O(n) porque solo se guarda `cabeza` → mejora pendiente: guardar también `cola` para O(1).
...
**Estado:** En progreso (Sesión 23: lista circular implementada y verificada; faltan lista genérica con iterador (plantillas), el `cola` para O(1) y el refactor de la lista simple a `unique_ptr`)
...
| 23 | 2026-09-25 | Fase 9 M1 (Cap. 10): **lista circular** (`ListaCircular.cpp`). Concepto: el último nodo apunta a la cabeza en vez de `nullptr`; el nodo único se apunta a sí mismo; lista vacía = `cabeza == nullptr`. `insertarFinal` ✓, `insertarInicio` ✓ (tres casos cada uno), `buscar` ✓, `imprimir` ✓ (repite la cabeza para mostrar el ciclo), `eliminar` ✓ (cabeza/intermedio/cola/último nodo) y destructor RAII ✓ — los dos últimos los terminó el tutor a pedido de Juan, explicados línea por línea. Compilación limpia con `g++ -Wall -Wextra -g`; valgrind 7 allocs/7 frees, 0 leaks, 0 errores. 🐛 (1) recorrer con `do{...}while(ultimo != cabeza)` termina en la CABEZA, no en la cola → el nodo nuevo se inserta tras la cabeza y el resto queda huérfano (pérdida silenciosa de datos, se detectó porque `buscar(2)` daba false; la condición correcta es `while(ultimo->siguiente != cabeza)`); (2) `while(actual != nullptr)` nunca termina en una circular, el centinela es `cabeza`; (3) `cabeza = cabeza->siguiente; delete cabeza;` borra la NUEVA cabeza y fuga la vieja → guardar `aBorrar` antes de mover; (4) al borrar la cabeza hay que reconectar el último a la nueva cabeza; (5) `nuevo = cabeza` solo mueve la variable local, no la lista (hay que tocar `->siguiente`); (6) destructor usando `cabeza` como caminante y como referencia de parada a la vez → para tras el primer nodo (3 fugados); (7) destructor sin guarda de lista vacía → crash. Copia deshabilitada (`= delete`) por doble free. Checkpoint 3/3: invariante del último, centinela `cabeza` vs `nullptr`, y caminante local distinto de la referencia de parada. | Completado |
---
